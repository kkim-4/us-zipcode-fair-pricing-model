...
# Naïve Baseline Data (Double brackets keep it as a DataFrame)
...
naive_xgb_model = xgb.XGBRegressor(
    ...
    max_depth=6, 
    subsample=0.8,
    colsample_bytree=1.0 # Must be 1.0 since there's only 1 feature
).fit(X_train_naive, y_train)


print(" Training Champion XGBoost (Full Infrastructure Suite)...")
...
def evaluate(model, X, y_log, label):
    ...
    # SAFETY CLIP: Prevents 'Infinity' errors from exploding linear coefficients
    y_pred_log = np.clip(y_pred_log, 9.2, 18.4)
    
    # Back-transform to dollars for MAE/RMSE
...
evaluate(lr_model, X_test, y_test, "Linear Regression (Full Features)")
...
# ==========================================
# 7. 2026 VALUATION AUDIT & CLASSIFIER
# ==========================================
...
def classify(delta):
    ...
    elif delta > 0.10: return 'Overvalued'
    else: return 'Fairly Priced'

df_clean['valuation_class'] = df_clean['fair_value_delta'].apply(classify)

# Filter for the most recent year
audit_2026 = df_clean[df_clean['YEAR'] == 2026].copy()

# Export for Geospatial Mapping
audit_2026.to_csv('USA_ZIP_VALUATION_AUDIT_2026.csv', index=False)
...
plt.rcParams['font.sans-serif'] = ['Inter', 'Arial', 'DejaVu Sans'] # Professional clean fonts
...
# Subtitle/Context (optional but professional)
...
ax.yaxis.set_major_formatter(mtick.StrMethodFormatter('{x:,.0f}')) # Add commas to Y-axis

# Remove spines for a modern "open" look
for spine in ['top', 'right', 'left']:
    ax.spines[spine].set_visible(False)
ax.spines['bottom'].set_color('#cbd5e1') # Lighten the bottom axis line
